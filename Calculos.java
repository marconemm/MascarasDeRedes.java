...
public class Calculos {// Cria a Classe "Hosts":
	...
	long SetHostsPossibles(long numLong, byte controller){ //Estabelece o Método "SetHostsPossible(l, controller), onde "l" é um parâmetro tipo Long e "controller" é tipo Byte".
		long hosts_p = 0, hosts_n = numLong;
	//	byte x = 0;
		
		switch (controller){
		case 0:
			if (hosts_n == 0){ 
				hosts_p = 0; // se o valor de hosts_n = 0, este método retorna 0.	
			} else {
					nZd = 0;
				...
				//	if (hosts_p > 254){
				//	hosts_p = (long)(Math.pow(2, nZd) - Math.pow(2, x)); //2^n	
				//	}
					}
			break;
		case 1:
			nZd = (byte)((numLong - 32)*(-1)); // determina o "numero de zeros decimais" de acrodo com o valor informado como CDIR no campo "textField_CDIR (numLong)".
			hosts_p = (long)(Math.pow(2,nZd) - 2);
			if (hosts_p == -1L){
				nZd = 1;
				hosts_p = 1L;
			}
